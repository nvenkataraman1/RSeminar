...
qb_stats <- list(name=c("Brett Favre", "Peyton Manning", 
                ...
                yards=c(71838, 69866, 61361, 56388, 53546, 123456))
qb_stats
```
...
+ Metadata: ```attributes```
+ Structure and summary: ```str``` and ```summary```
+ Names: ```names```, ```rownames```, ```colnames```
+ Size and Dimensionality: ```length``` and ```dim```
+ Type of data: ```class``` and ```typeof```
+ Quick view: ```head``` and ```tail```
...
```{r}
c(list(1:10),
  list(sample(c(T,F), size = 100, replace = T)),
  list(LETTERS))
```
